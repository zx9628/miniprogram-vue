    ...
    <view class="modal-content" @click.stop>
      ...
      <view class="spec-section">
        ...
        <view class="spec-group">
          ...
          <view class="spec-options flavor-options">
            <view
                v-for="(item, index) in flavorOptions"
                :key="index"
                class="spec-option flavor-option"
                :class="{ active: localSelectedFlavors.includes(index) }"
                @click="toggleFlavor(index)"
            >
              {{ item.name }}
            </view>
          </view>
      ...
      <view class="modal-footer">
        ...
        <view class="add-cart-btn" @click="handleAddToCart">
          {{ selectedSpecName }}{{ selectedFlavorName ? '/' + selectedFlavorName : '' }}
          <text v-if="selectedSpecName || selectedFlavorName">·</text>
          加入购物车
        </view>
      </view>
    </view>
...
const localSelectedFlavors = ref([])
...
const selectedFlavorName = computed(() => {
  if (localSelectedFlavors.value.length === 0) return ''
  return localSelectedFlavors.value.map(i => props.flavorOptions[i]?.name).join('/')
})
...
const toggleFlavor = (index) => {
  ...
  if (idx > -1) {
    localSelectedFlavors.value.splice(idx, 1)
  } else {
    localSelectedFlavors.value.push(index)
  }
}
...
const handleAddToCart = () => {
  const order = {
    ...
    flavors: localSelectedFlavors.value.map(i => props.flavorOptions[i]?.name).filter(Boolean),
    ...
    selectedFlavorIndexes: [...localSelectedFlavors.value],
